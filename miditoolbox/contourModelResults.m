function contourModelResults()
    materials = load('contour-poll-materials.mat');
    results = load('contour-poll-results.mat');
    n_mel = length(materials.mel_midi);
    mel_rep = cell(1, n_mel);
    for i = 1:n_mel
        mel_rep{i} = contourBuildRepresentation(materials.mel_midi{i});
    end
    ...
    for i = 1:n_mel-1
        for j = i+1:n_mel
            ...
            sim_temp = contourScoreSimilarity(mel_rep{i}, mel_rep{j});
    ...
    for i = 1:n_mel
        sim_res_disp{1, i + 1} = ['Contour ', num2str(i)];
        sim_res_disp{i + 1, 1} = ['Contour ', num2str(i)];
        sim_pred_disp{1, i + 1} = ['Contour ', num2str(i)];
        sim_pred_disp{i + 1, 1} = ['Contour ', num2str(i)];
        for j = 1:n_mel
